...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for input_batch, target_batch in train_loader:
            optimizer.zero_grad()
            loss: torch.Tensor = evaluator.calculate_batch_loss(input_batch=input_batch, target_batch=target_batch)
            loss.backward()
            optimizer.step()
            ...
            if global_step % eval_freq == 0:
                train_loss, val_loss = evaluator.evaluate_model(train_loader=train_loader, validation_loader=validation_loader, eval_iter=eval_iter)
        ...
        text_produced: str = text_provider.produce_text(start_context=start_context)
